...
public class ThinkNodeFcUP3 extends ThinkNodeFcUP {

	public ThinkNodeFcUP3(int turn, Field field, long score, int fcp1, int isFire, int x, int r) {
	...
	public ThinkNode getNextNode(){
		...
		if(first instanceof ThinkNodeFcUP3){
			if(((ThinkNodeFcUP3)first).isFire == 1){
				max_child = first;
			}else{
				int[] min_turn = {CodeVs.TURN_NUM, 0};
				for(int i=0; i<child.size(); i++){
					ThinkNodeFcUP3 node = (ThinkNodeFcUP3) child.get(i);
					int[] temp = new int[2];
					node.fireTurn(temp);
...
//			if((this.brock_num - node.brock_num) < 50){
	...
	protected void fireTurn(final int ret[]){
		...
		if(this.turn < CodeVs.TURN_NUM-1 && turn_dif < 3){
			...
			if(child.size() != 0 && child.get(0) instanceof ThinkNodeFcUP3){
				if(((ThinkNodeFcUP3)child.get(0)).isFire == 1){
					ret[0] = this.turn + 1;
					ret[1] = ((ThinkNodeFcUP3)child.get(0)).brock_num;
					return;
				}else{
					int[] min_turn_ret = {CodeVs.TURN_NUM,0};
					for(int index = 0; index<child.size(); index++){
						ThinkNodeFcUP3 node = (ThinkNodeFcUP3) child.get(index);
						int temp[] = new int[2];
						node.fireTurn(temp);
	...
	protected void setChildFcUP(){
		ThinkNodeFcUP3 fireNode = null;
		
		ArrayList<ThinkNodeFcUP3> temp_list = new ArrayList<ThinkNodeFcUP3>();
		...
		for(int x = 1 - CodeVs.Pack_size; x<CodeVs.Field_wid; x++){
			for(int r = 0; r<4; r++){
				...
				if(raw_score > -1){
					ThinkNodeFcUP3 node = null;
					long score = raw_score * Fcp1;
					if(raw_score >= CodeVs.Th){
						node = new ThinkNodeFcUP3(this.turn+1, temp, score, Fcp1 + 1, 1,x,r);
						if(fireNode == null){
							fireNode = node;
						}else{
						...
						//else{
						node = new ThinkNodeFcUP3(this.turn+1, temp, score, Fcp1, 0,x,r);
					}
