...
public class ThinkNode6_1 extends ThinkNode6{
	
	
	public ThinkNode6_1(int turn, Field field, long score, int fcp1, int x, int r) {
	...
	public long getMaxScore(){
		...
		if(this.turn < CodeVs.TURN_NUM-1 && turn_dif < 5){				// 読む手数を変えるにはここを変える。setUP()も。
			if(child.size() == 0){
				setChild();
			}
			for(int index = 0; index<child.size(); index++){
				ThinkNode6 node = (ThinkNode6) child.get(index);
				if(2*(this.brock_num - node.brock_num) > Math.pow(this.brock_num, 2)/(CodeVs.Field_wid * CodeVs.hei_p_size)){// 消えたブロック数が多いものはゴールとして、移行の手を見ない
