...
public class ThinkNode_l_FcUP extends ThinkNodeFcUP{

	public ThinkNode_l_FcUP(int turn, Field field, long score, int fcp1, int isFire, int x, int r) {
		super(turn, field, score, fcp1, isFire, x, r);
	}
	
	public long setChild(){
		if(this.turn - CodeVs.Turn < 3){
			if(this.turn<600){							// 切り替えポイント
				setChildFcUP();
			}else{
				setChildNormal();
			}
	...
	public ThinkNode getNextNode(){
		...
		if(first instanceof ThinkNode_m_FcUP){
			if(((ThinkNode_m_FcUP)first).isFire == 1){
				max_child = first;
			}else{
				int[] min_turn = {CodeVs.TURN_NUM, 0};
				for(int i=0; i<child.size(); i++){
					ThinkNode_m_FcUP node = (ThinkNode_m_FcUP) child.get(i);
					int[] temp = new int[2];
					node.fireTurn(temp);
...
//			if((this.brock_num - node.brock_num) < 50){
	...
	protected void fireTurn(final int ret[]){
		...
		if(this.turn < CodeVs.TURN_NUM-1 && turn_dif < 3){
			...
			if(child.size() != 0 && child.get(0) instanceof ThinkNode_l_FcUP){
				if(((ThinkNode_l_FcUP)child.get(0)).isFire == 1){
					ret[0] = this.turn + 1;
					ret[1] = ((ThinkNode_l_FcUP)child.get(0)).brock_num;
					return;
				}else{
					int[] min_turn_ret = {CodeVs.TURN_NUM,0};
					for(int index = 0; index<child.size(); index++){
						ThinkNode_l_FcUP node = (ThinkNode_l_FcUP) child.get(index);
						int temp[] = new int[2];
						node.fireTurn(temp);
	...
	protected void setChildFcUP(){
		ThinkNode_l_FcUP fireNode = null;
		
		ArrayList<ThinkNode2> temp_list = new ArrayList<ThinkNode2>();
		...
		for(int x = 1 - CodeVs.Pack_size; x<CodeVs.Field_wid; x++){
			for(int r = 0; r<4; r++){
				...
				if(raw_score > -1){
					ThinkNode_l_FcUP node = null;
					long score = raw_score * Fcp1;
					if(raw_score >= CodeVs.Th){
						node = new ThinkNode_l_FcUP(this.turn+1, temp, score, Fcp1 + 1, 1,x,r);
						if(fireNode == null){
							fireNode = node;
						}else{
						...
						//else{
						node = new ThinkNode_l_FcUP(this.turn+1, temp, score, Fcp1, 0,x,r);
					}
...
//		LinkedList<ThinkNode_l_ScoreUP> temp_list = new LinkedList<ThinkNode_l_ScoreUP>();
