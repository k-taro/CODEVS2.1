...
public class ThinkNode_m_FcUP extends ThinkNodeFcUP {

	public ThinkNode_m_FcUP(int turn, Field field, long score, int fcp1, int isFire, int x, int r) {
...
//			if((this.brock_num - node.brock_num) < 50){
	...
	protected void fireTurn(final int ret[]){
		...
		if(this.turn < CodeVs.TURN_NUM-1 && turn_dif < 3){
			...
			if(child.size() != 0 && child.get(0) instanceof ThinkNode_m_FcUP){
				if(((ThinkNode_m_FcUP)child.get(0)).isFire == 1){
					ret[0] = this.turn + 1;
					ret[1] = ((ThinkNode_m_FcUP)child.get(0)).brock_num;
					return;
				}else{
					int[] min_turn_ret = {CodeVs.TURN_NUM,0};
					for(int index = 0; index<child.size(); index++){
						ThinkNode_m_FcUP node = (ThinkNode_m_FcUP) child.get(index);
						int temp[] = new int[2];
						node.fireTurn(temp);
	...
	protected void setChildFcUP(){
		ThinkNode_m_FcUP fireNode = null;
		...
		for(int x = 1 - CodeVs.Pack_size; x<CodeVs.Field_wid; x++){
			for(int r = 0; r<4; r++){
				...
				if(raw_score > -1){
					ThinkNode_m_FcUP node = null;
					long score = raw_score * Fcp1;
					if(raw_score >= CodeVs.Th){
						node = new ThinkNode_m_FcUP(this.turn+1, temp, score, Fcp1 + 1, 1,x,r);
						if(fireNode == null){
							fireNode = node;
						}else{
						...
						//else{
						node = new ThinkNode_m_FcUP(this.turn+1, temp, score, Fcp1, 0,x,r);
					}
